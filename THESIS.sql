  ...
  CREATE OR REPLACE FORCE VIEW "PATCH_MD"."V_MRN_THESIS" ("WORKITEM_ID", "FINISH_COLLECT_DATE", "HITCH", "EVENT_DATE", "ENV") AS 
  WITH
    BPV AS (
...
MT AS (SELECT 
WORKITEM_ID, FINISH_COLLECT_DATE, (MIN(MT_START_INSTALL_DATE) - MAX_INST_FINISH_INSTALL_DATE) AS MT_HITCH, MIN(MT_START_INSTALL_DATE) AS MT_START_INSTALL_DATE, MAX_INST_FINISH_INSTALL_DATE
FROM
(SELECT 
...
FROM (SELECT WORKITEM_ID, FINISH_COLLECT_DATE, START_INSTALL_DATE FROM BPV    
        WHERE ENVIRONMENT = 'MT'
        ) MT_1
JOIN (
        SELECT WORKITEM_ID, FINISH_COLLECT_DATE, FINISH_INSTALL_DATE FROM BPV    
        WHERE ENVIRONMENT = 'INST'
        AND INSTALL_CODE = '0'
        ) INST_1
...
SIT AS (SELECT 
WORKITEM_ID, FINISH_COLLECT_DATE, (MIN(SIT_START_INSTALL_DATE) - MAX_MT_FINISH_INSTALL_DATE) AS SIT_HITCH, MIN(SIT_START_INSTALL_DATE) AS SIT_START_INSTALL_DATE, MAX_MT_FINISH_INSTALL_DATE
FROM
(SELECT 
